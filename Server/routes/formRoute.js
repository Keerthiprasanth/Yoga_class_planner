...
router.get("/class-forms/:classId", async (req, res) => {
  try {
    ...
    const forms = await FormData.find({ submittedBy: studentIds })
    .populate({
      path: "submittedBy",
      select: "name email",
    })
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      const sequenceIndex = formData.suggestedSequences.findIndex(
        (suggestion) =>
      ...
      if (sequenceIndex === -1) {
        return res
          ...
          .json({
            message:
              "Sequence not found in the form or not suggested by the teacher",
          });
      }
      ...
      res
        .status(200)
        .json({ message: "Sequence removed from the form successfully" });
    } catch (error) {
